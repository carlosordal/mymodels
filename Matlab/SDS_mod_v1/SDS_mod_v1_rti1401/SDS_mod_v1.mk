...
MATLAB_ROOT_ALT                = C:\PROGRA~1\MATLAB\R2018b
MATLAB_BIN_ALT                 = C:\PROGRA~1\MATLAB\R2018b\bin
RELEASE_VERSION                = R2018b
RELEASE_VERSION               := $(RELEASE_VERSION,S'\.'_',W_,LC,S'^r'')
COMPUTER                       = PCWIN64
MODEL                          = SDS_mod_v1
MODULES                        = SDS_mod_v1_data.c SDS_mod_v1_trc_ptr.c
S_FUNCTIONS                    = 
S_FUNCTIONS_LIB                = 
SOLVER                         = 
NUMST                          = 2
...
COMP_ROOT = "$(DSPACE_CONFIG)\Compiler\PPCTools"
...
start_check :
	...
	%if !(exists("$(CC,"=)") && exists("$(AS,"=)") && exists("$(LD,"=)") && exists("$(AR,"=)"))
	  ...
	  %echo        - Environment variable DSPACE_ROOT\DSPACE_CONFIG points to wrong directory.
	  %do make_abort
	%endif
	%if !(dir("$(MATLAB_BIN,"=)"))
